...
cap = cv2.VideoCapture('1_360p.avi')
size = (
    int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
    int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
)
codec = cv2.VideoWriter_fourcc(*'DIVX')
output = cv2.VideoWriter('output.avi', codec, 25.0, size)
...
while (cap.isOpened()):
    ...
    if ret:
        if i % frame_rate_divider == 0:
            ...
            log("masks", r['masks'])
            log("scores", r['scores'])
            result_frame = cv2_display_keypoint(frame, r['rois'], r['keypoints'], r['masks'], r['class_ids'],
